 ...
 * Date: 		Apr.18th
 * 
 * VigenereCipher is the class, subclass of CryptographGeneral, encrypt the text 
 * in Vigenere cipher
 * 
 * Vigenere Cipher is a cryptosystem that giving a keyword, each letter represents a 
 * number, shift each letter in the order of the given keyword
 * 
 */
import java.util.ArrayList;

public class VigenereCipher extends CryptographGeneral
{
	private ArrayList<Integer> kWCode; // ArrayList of integer of
	private ArrayList<Character> textChange; // ArrayList of the text

	// constructor
	public VigenereCipher(String iT, String kW)
	{
		super(iT, kW);
	...
	{// take the keyword string and convert the valid characters into the
		...
		if (kWCode.isEmpty()) // if nothing is added, then add 0 as the shifting
			kWCode.add(0);
		System.out.println("The keyword: "+keyWord+"\nVigenere shift:"+kWCode);
	}

	public void Cryptogam()
	{ // the method encrypt the String by keyword
		...
		{// for every character in the ArrayList
			codeAdd = kWCode.get(index % kWCode.size());// find the
														...
														// to shift
			codeC = (int) (textChange.get(index));// and convert the character
													// to integer
			// the next if else if statement identify the character, weather
			// they are still letter but not non-letter characters
			if (codeC >= 65 && codeC <= 90 && codeC + codeAdd > 90)
				codeFAdd = codeAdd + codeC - 26;
			else if (codeC >= 97 && codeC <= 122 && codeC + codeAdd > 122)
				codeFAdd = codeAdd + codeC - 26;
			else
				codeFAdd = codeAdd + codeC;
